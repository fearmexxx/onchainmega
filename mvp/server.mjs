...
const allowedRpcMethods = new Set(["getBalance", "getLatestBlockhash", "getSignatureStatuses", "getTransaction", "sendTransaction"]);
...
createServer(async (request, response) => {
  if (request.url === "/api/rpc" && request.method === "POST") {
    let raw = "";
    ...
    try {
      ...
      if (!allowedRpcMethods.has(payload.method)) {
        response.writeHead(403, { "content-type": "application/json" }).end(JSON.stringify({ error: "RPC method not allowed" }));
        return;
      }
      ...
      response.writeHead(502, { "content-type": "application/json" }).end(JSON.stringify({ error: "Devnet RPC unavailable", detail: error.message }));
    }
    return;
  }
